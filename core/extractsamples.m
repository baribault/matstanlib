...
if nargin < 1 || ~ischar(interface)
    ...
    if ~ismember(interface,validInterfaces)
        error(['the interface input ''%s'' was not recognized. ' ...
            'valid Stan interface names include: ''%s'''], ...
...
switch interface
    ...
    case 'matlabstan'
        if nargin < 2
            error(['to process output from the MATLABStan interface, ' ...
                'a StanFit object is required.  ' ...
                'your call to this function should look like:%s\n' ...
                '>> extractsamples(''matlabstan'',fit)'],'')
        end
    % ------------------------------------------------------------------- %
    case 'trinity'
        if nargin < 3
            error(['to process output from the Trinity interface, ' ...
                'both a ''chains'' structure and an ''info'' structure ' ...
                'are required.' ...
                'your call to this function should look like:%s\n' ...
