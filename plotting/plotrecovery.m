...
switch credIntType
    case {'CI','CI95'}
        %95% central interval
        lowerBound = 'lowerCI95';
        upperBound = 'upperCI95';
    case 'CI50'
        %50% central interval
    ...
    case 'HDI'
        ...
        lowerBound = 'lowerHDI95';
        upperBound = 'upperHDI95';
end
...
for n = 1:length(parameters)
    ...
    if any(inCI) && makeErrorBars
        g = plot([trueVals(inCI) trueVals(inCI)]', ...
            ...
            'marker','none','linewidth',linePt,'color',paramColors(n,:));
        %transparent
        for ng = 1:length(g)
            g(ng).Color(4) = 0.5;
        end
...
if ~isempty(highlightValue)
    ...
    plot(recov,recov,'k:')
    %highlight value
    plot(highlightValue*[1 1],recov,'color','k','linewidth',linePt,'linestyle',':')
    plot(recov,highlightValue*[1 1],'color','k','linewidth',linePt,'linestyle',':')
end
