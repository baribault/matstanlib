function essintervalplot(samples,diagnostics,parameterRequest)
%ESSINTERVALPLOT generates the "local efficiency" diagnostic ESS plot.
% 
% this function visualizes the "local efficiency of small-interval
% probability estimates" plot recommended by Vehtari et al. (2020), one of
% three new diagnostic plots based on effective sample size (ESS)
% estimates. 
% 
% visualizing whether ESS is notably lower in some regions (e.g., much
% lower ESS in the tails), especially if those regions seem to correlate
% with divergent transitions or hitting max tree depth, can help identify
% what areas of the marginal posterior are driving low ESS.  
% 
...
% 
% 
% ESSINTERVALPLOT(SAMPLES,DIAGNOSTICS,PARAMETERREQUEST)
...
%% make a "local small-interval efficiency" plot for each parameter
...
for p = 1:min([length(parameters),maxNfigures])
    ...
    plot(xIntervalPos,repelem(ESSbyQuantile,2),'-', ...
        'linewidth',linePt*1.25,'color',0.9*[1 1 1])
