function violindensity(samples,parameterRequest,varargin)
%VIOLINDENSITY generates a violin plot for selected parameters.
% 
% this function generates a violin plot of one or more parameters. 
% on each violin, the outlined, filled area is a mirrored, smoothed
% univariate marginal density, overlaid on which is a thin line
% representing the central 95% credible interval, a thick line representing
% the central 50% credible interval, and a marker representing the median.
% a horizontal dashed line across the violin represents the preferred point
% estimate, the mean of the posterior samples. 
% this default appearance may be customized using the optional inputs.
% 
% 
% VIOLINDENSITY(SAMPLES,PARAMETERREQUEST)
...
%   for each parameter instance, a violin density is added to the plot. 
% 
% 
% optional inputs may be given as property name-value pairs:
% VIOLINDENSITY(SAMPLES,PARAMETERREQUEST,'name1',VALUE1,'name1',VALUE2,...)
...
%       if true, a horizontal dotted line spanning the width of the violin
...
%       interval plot is overlaid at the midline of the violin. 
...
%       supplying CRITICALVALUE triggers the underlay of a horizontal line
%       at the given value.  
%   
%   'fillcolor'
...
%       plot under the midline of each violin.
...
% 
% 
% See also HORZDENSITY, OVERLAYDENSITY, PLOTINTERVALS, PLOTDENSITY, 
...
fpos = f.Position;
f.Position = [fpos([1 2]) [100+60*(nParameters) 480]*figScaling];
close(dumf.Number); %close dummy figure
%... and an axis
hold on

%what proportion of horizontal space should be taken up by 
scaleF = 0.95;      % ... the density (at maximum width)?
...
scaleF = scaleF/2; %halve it for easier use
for n = 1:nParameters
    ...
    if outline > 0
        %outline L side & R side
        for side = [-1 1]
            fv = f(inOutline); xv = x(inOutline);
            plot(n+side*fv,xv,'color',outlineColors(n,:),'linewidth',linePt)
        end
        %connect L & R outline with horizontal ends
        plot([n-fv(1) n+fv(1)],xv(1)*[1 1], ...
            'color',outlineColors(n,:),'linewidth',linePt)
        plot([n-fv(end) n+fv(end)],xv(end)*[1 1], ...
            'color',outlineColors(n,:),'linewidth',linePt)
    end
    ...
    if addEstimate
        ...
        plot([n-f(indMean) n+f(indMean)],m*[1 1],':','color',outlineColors(n,:))
    end
    %overlay ...
    if addInterval
        ...
        %... median (marker)
        plot(n,mdn,'o','markersize',linePt*4, ...
            'markerfacecolor',markerColors(n,:), ...
            'markeredgecolor',outlineColors(n,:))
    end
    %overlay true value (star marker, on median)
    if addTrueValues
        plot(n,tr, ...
            'marker','p','markersize',markSz*1.25, ...
            'markerfacecolor',markerColors(n,:),'markeredgecolor',trueColors(n,:))
    end
end
%format plot
set(gca,'xlim',[0.25 nParameters+0.75], ...
    'xtick',1:nParameters,'xticklabels',parameters)
if ~isnan(criticalValue)
    if criticalValue > maxX, maxX = criticalValue; end
    if criticalValue < minX, minX = criticalValue; end
end
bufferX = 0.05*(maxX-minX);
ylim([minX-bufferX maxX+bufferX])
set(gca,'ticklabelinterpreter','none')
if max(cellfun(@length,parameters)) > 6
    set(gca,'xticklabelrotation',30)
end
set(gca,'xgrid',gridOnOff,'box','on','fontsize',fontSz)
